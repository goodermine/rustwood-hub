...
export default function NewsletterSignup({ variant = "full" }: NewsletterSignupProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    toast.success("Thanks for joining! Newsletter launching soon — you'll be first to know.");
    setEmail("");
  }
  ...
  if (variant === "compact") {
    return (
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-md mx-auto">
        ...
        <button
          type="submit"
          className="px-5 py-2.5 text-xs font-bold tracking-widest uppercase transition-all duration-300 hover:shadow-[0_0_20px_rgba(212,168,67,0.3)] whitespace-nowrap"
          style={{
